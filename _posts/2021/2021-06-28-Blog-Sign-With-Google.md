---
title: 给你的博客添加Sign With Google登录按钮（本文已过时）
date: "2021-06-18 18:40:00"
update: "2024-10-31 07:14"
tags: [HTML, docs]
category: blog
---
今天发现第三方网站可以使用Google的登录接口，便研究了一下如何接入以自动获取基本信息，在这里介绍一下流程和踩的坑。

<!-- more -->

2024-10-31更新：谷歌已经不再使用该方案，请参考新文档。
...
3. 点击左侧的Credentials，上面会有一个Create Credentials按钮，点击后会有一个菜单，点击OAuth cilent ID，选择Web application，填好内容，这时会得到一个二级域名是googleusercontent.com的域名，复制下来。
...
5. 想自定义登录按钮可以看第四条指南，但是官方文档写的很迷惑人，删减后的代码我贴在这里：
   ...
   element是DOM节点，必须能够添加click事件。

最坑的地方来了，如果你仅仅找到了前四个指南，你会发现你在全局作用域是获取不到登录信息的，那个googleUser的变量就是摆设。
...
`auth2.currentUser.get()`，这个方法返回一个googleUser对象，可以这样用：
