...
如果一个类提供了移动构造或者移动赋值，并且在构造或者赋值时使用了std::move，那么static\_cast就会调用对应的具有移动语义的重载函数；如果一个类只能移动赋值，不能复制，那么static\_cast会直接选择移动赋值。
