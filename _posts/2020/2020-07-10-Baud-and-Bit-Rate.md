...
00011011这组信号有8bit，单从信号看，只表示0，1两种状态。现在现在认为每两个bit为一个有效信号，即00 01 10 11。那么这组信号就可以表示2 <sup>2</sup> 种状态，变成0 1 2 3。有效信号可以通过调制负载多个比特。

## 换算公式

现在可以得到 比特率 = 波特率 \* log <sub>2</sub> (L)，其中L为每个有效信号负载的信息量（Symbol）。
