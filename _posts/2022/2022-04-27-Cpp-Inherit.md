...
为了解决这个问题，STL的库设计者通常使用这种设计：将 `allocator` 设计为 `vector_base` 的基类。此时若 `allocator` 为无数据成员的 `std::allocator`，同时由于**无数据成员的基类可和派生类重叠**，导致 `std::allocator` 的空间可以被优化掉，这种设计叫做空基类优化。
...
从继承的访问权限设计本身来说，`public` 表示了一种is-a的关系，如果该关系不是is-a，那么就不适用于 `public`。
...
C++20添加了 `[[no_unique_address]]` 属性，用于指示数据成员可以被覆盖，可以直接做到空基类优化的效果。
