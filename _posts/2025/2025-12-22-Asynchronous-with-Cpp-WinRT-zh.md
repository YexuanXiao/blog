...
在C++/WinRT中创建委托的方式有很多种，C++/WinRT允许lambda, 函数指针和成员指针作为委托，使用成员指针时，还需要额外传递一个对象。该模式非常经典，但当委托是协程时，有额外的问题需要注意。
...
在编写协程委托时，如果参数需要在发生线程切换后使用，那么就需要传递值而不是引用。在我的协程教程中讲过，协程的状态（协程帧）储存参数列表，因此参数列表中的对象在整个协程可见的生存期内都有效。如果某个参数不在函数体内使用，那么也可以写成引用。
